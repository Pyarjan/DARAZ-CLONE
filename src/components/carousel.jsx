...
const Carousel = ({ slides }) => {
  ...
  return (
    ...
    >
      ...
      >
        {slides.map((s, index) => (
          <img key={index} src={s} className="w-full" alt={`Slide ${index}`} />
        ))}
      </div>

      <div className="absolute top-0 w-full h-full justify-between flex text-white text-xl px-1">
      ...
      <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-2">
        {slides.map((_, index) => (
          <div
            ...
            className={`w-3 h-1 rounded-full ${current === index ? 'bg-orange-500' : 'bg-white'} cursor-pointer`}
          />
        ))}
      </div>
    </div>
